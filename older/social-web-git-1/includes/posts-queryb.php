<?php 
	...
	if(isset($_POST['sub-post'])){
		...
		if(!mysqli_stmt_prepare($stmtq, $qstatement1)) {
			...
			$res=0;
			for($i = 0;$i = $res; $i = $i){
				$sqlc = "SELECT * FROM posts WHERE user_id=? AND domain_num=? AND author=? AND title=? AND body=?";
				...
				if(!mysqli_stmt_prepare($stmtc, $sqlc)) {
					...
					mysqli_stmt_bind_param($stmtc, "sssss",$userpost, $domain_num, $pauthor, $ptitle, $pbody);
